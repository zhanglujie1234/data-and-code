setwd("G:/NJ+10个样地/物种组成/αβ多样性")
library(ggplot2)
library(ggbreak)
dat<-read.table("柱状图-4.csv", header=T, sep=",")
dat1 <- dat[,c(1:2)]
b <- colnames(dat)
#不同的指数作图调整d值
d <- 1
...
#cbbPalette1<-c('#cd9e36','#f4c1b3','#ed877b','#b5544d')
...
p1 <- ggplot(dat1,aes(taxonomy,mean,fill = taxonomy)) +
  ...
  scale_y_continuous(breaks = seq(0,3000, by = 2),expand = c(0,0.3))+
  #coord_flip() +
  xlab("") +
  ylab(c)+
...
p1 <- p1 + 
  ...
  geom_text(aes(label = sign1, y = mean + se +0.2), position = position_dodge(0.5),size=4,family="sans")+

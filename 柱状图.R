...
dat<-read.table("柱状图-5.csv", header=T, sep=",")
dat1 <- dat[,c(1:2)]
b <- colnames(dat)
#不同的指数作图调整d值
d <- 1
...
cbbPalette1<-c('#d78e8b','#aeb6b8','#d78e8b','#e7b65b','#71b476','#116fb2')

dat1$taxonomy<-factor(dat1$taxonomy,levels = c("Antarctica1","Forest"," ","Antarctica","Temperate","Subtropic","Tropic"))
p1 <- ggplot(dat1,aes(taxonomy,mean,fill = taxonomy)) +
  #这句注意，fill=group，是一个处理两根柱子的关键
  scale_x_discrete(limits = levels(dat1$taxonomy)) +
  scale_y_continuous(breaks = seq(0,3, by = 1),expand = c(0,0.05))+
  #coord_flip() +
  xlab("") +
  ylab(c)+
...
p1 <- p1 + 
  ...
  geom_text(aes(label = sign1, y = mean + se + 0.06), position = position_dodge(0.5),size=4,family="sans")+
  scale_fill_manual(values=cbbPalette1,guide = guide_legend(reverse = TRUE))

p1

ggsave(filename = "6个柱子.pdf", plot = p1, width = 5.5, height = 4)
